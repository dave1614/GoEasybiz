          ...
          <table>
            ...
            <tbody>
              <tr v-for="(row,index) in wallet_statement.data" :key="row.id">
                ...
                <td :class="row.amount_after > row.amount_before ? 'text-green-500' : 'text-red-500'"
                  data-label="Amount"
                  v-html="row.amount_after > row.amount_before ? '+' + mainStore.addCommas(row.amount) : '-' + mainStore.addCommas(row.amount)">
