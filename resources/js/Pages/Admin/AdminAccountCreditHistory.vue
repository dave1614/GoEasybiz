<template inheritAttrs="false">
  <LayoutAuthenticated>
    ...
    <Head :title="`Admin Credit History`" />
    <SectionMain>
      <SectionTitleLineWithButton :icon="mdiClipboardTextClock" :title="`Admin Credit History`" main>
          ...
          <table>
            ...
            <tbody>
              <tr v-for="(row,index) in history.data" :key="row.id">
                ...
                <td class="text-xs text-primary" data-label="Email">
                  {{ row.email }}
                </td>
...
const submitFilterForm = () => {
  ...
  let queryRoute = route('admin_account_credit_history', user.id);
