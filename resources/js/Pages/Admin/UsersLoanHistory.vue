<template inheritAttrs="false">
  <LayoutAuthenticated>
    ...
    <Head :title="`${user.name} Easy Loan History`" />
    <SectionMain>
      <SectionTitleLineWithButton :icon="mdiClipboardTextClock" :title="`${user.name} Easy Loan History`" main>
      ...
      <CardBox isForm @submit.prevent="submitFilterForm" class="">
        <div class="sm:grid sm:grid-cols-12 sm:gap-6">
          ...
          <FormField class="sm:col-span-4" label="Amount Loaned">
            <FormControl v-model="form.amount" type="number" step="any" />
          </FormField>
          <FormField class="sm:col-span-4" label="Amount Paid">
            <FormControl v-model="form.amount_paid" type="number" step="any" />
          </FormField>
          
          <FormField class="sm:col-span-4" label="Loan Date">
            <FormControl v-model="form.date" type="date" />
          </FormField>
          <FormField class="sm:col-span-4" label="Last Date Paid">
            <FormControl v-model="form.last_date_paid" type="date" />
          </FormField>
          ...
          <table>
            ...
            <tbody>
              <tr v-for="(row, index) in history.data" :key="row.id">
                ...
                <td data-label="Amount Loaned" v-html="mainStore.addCommas(row.amount)">
              
                </td>
                <td data-label="Amount Paid" v-html="mainStore.addCommas(row.amount_paid)">
              
                </td>
                <td data-label="Amount Owed" v-html="mainStore.addCommas(row.amount_owed)">
              
                </td>
                <td data-label="Date / Time" v-html="`${row.date}`">
              
                </td>
                <td data-label="Last Date Paid" v-html="`${row.last_date_paid}`">
...
const form = useForm({
  ...
  amount_paid: props.value?.amount_paid,
  last_date_paid: props.value?.last_date_paid,
...
const submitFilterForm = () => {
  ...
  let queryRoute = route('users_easy_loan_history', user.id);
