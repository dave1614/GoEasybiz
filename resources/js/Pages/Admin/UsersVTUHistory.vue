<template inheritAttrs="false">
  <LayoutAuthenticated>
    ...
    <Head :title="`${user.name}'s VTU History`"/>
    <SectionMain>
      <SectionTitleLineWithButton :icon="mdiHistory" :title="`${user.name}'s VTU History`" main>
...
const form = useForm({
  ...
  sub_type: props.value?.sub_type,
  order_id: props.value?.order_id,
...
const submitFilterForm = () => {
  ...
  let queryRoute = route('users_vtu_history', user.id);
