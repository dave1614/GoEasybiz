<template inheritAttrs="false">
  <LayoutAuthenticated>
    ...
    <Head :title="`${user.name}'s Account Credit History`" />
    <SectionMain>
      <SectionTitleLineWithButton :icon="mdiClipboardTextClock" :title="`${user.name}'s Account Credit History`" main>
    ...
    <FloatingActionButton :title="'Go Back'" @click="Inertia.visit(route('view_members_list') + '?length=10&name='+ user.name +'&isDirty=true&__rememberable=true')">
...
const submitFilterForm = () => {
  ...
  let queryRoute = route('users_account_credit_history', user.id);
