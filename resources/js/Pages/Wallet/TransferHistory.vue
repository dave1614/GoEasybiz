<template inheritAttrs="false">
  <LayoutAuthenticated>
    ...
    <Head :title="`Funds Transfer History`" />
    <SectionMain>
      <SectionTitleLineWithButton :icon="mdiCashRefund" :title="`Funds Transfer History`" main>
          ...
          <table>
            ...
            <tbody>
              <tr v-for="(row,index) in history.data" :key="row.id">
                ...
                <td data-label="Role" :class="row.sender == user.id ? 'text-red-500' : 'text-primary'" class="text-xs italic" v-html="row.sender == user.id ? 'Sender' : 'Recepient'">
...
const submitFilterForm = () => {
  ...
  let queryRoute = route('transfer_history', user.id);
